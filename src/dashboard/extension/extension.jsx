...
import ProNotice from '../pro-model/pro-notice.jsx';
...
const Extension = (props) => {
    ...
    const InstallPopup = () => {
        return (
            <div className="tpae-nexter-content">
                ...
                <div className="tpae-nexter-accordion">
                    {nexter_poup_accordian.map((item, index) => (
                        <div key={index} className="nexter-accordion-item">
                            <div className={`nexter-accordio-content ${activeIndex === index ? "tpae-content-opan" : ""}`} onClick={() => handleToggle(index)}>
                                <div className="tpae-accd-qui">{item.question}</div>
                                <div className='tpae-pm-lable'>
                            ...
                            <div className={`tpae-accordion-ans ${activeIndex === index ? "opan-accordion" : ""}`}>{item.answer}</div>
                        </div>
                    ))}
                </div>
            </div>
            ...
            <>
                {
                    extension_json.plus_extension.map((categoryKey, index) => {
                        ...
                        if (extension_options.length > 0) {
                            ...
                            return (
                                <div className='tpae-catwise-wid-manage-cov' key={index}>

                                    <div className='tpae-catwise-wid-mancover'>
                                        <div className='tpae-wid-h-infogrp'>
                                            <h3 className='tpae-wid-cat-h' id={'she_' + categoryKey.name}>
                                                {categoryKey.label}
                                            </h3>
                                    ...
                                    <div className='tpae-wid-box-cover'>
                                        {extension_options.map((data, index) => {
                                            return (
                                                <div className={`tpae-part-wid-mng-box ${(data?.tag == 'pro') && !free_pro ? 'tpae-widget-pro' : ''}`} key={index}>
                                                    <div className='tpae-wid-in-left'>
                                                        <div className={`tpae-nexter-tag`}>{data.tag}</div>
                                                        <div className='tpae-wid-name'>{data.label}
                                                            {pro_plugin == '0' && data.status &&
                                                                <span className={`tpae-wid-sm-tag tpae-wid-name-hint-text tp-tag-color-${data.status}`}>{data.status}</span>
                                                            }
                                                            {pro_plugin == '1' && data.pro_status &&
                                                                <span className={`tpae-wid-sm-tag tpae-wid-name-hint-text tp-tag-color-${data.pro_status}`}>{data.pro_status}</span>
                                                            }
                                                        </div>
                                                    ...
                                                    <div className='theplus-widget-inner-toggle'>
                                                        <label className="tpae-nexter-tgl-btn-cover">

                                                            {!Nextercheck ? (
                                                                <div className="nexter-download-template" onClick={() => NexterPopup(true)}>
                                                                ...
                                                                <a className='tpae-nexter-redirect' href={nexter_Plugin_url()} target="_blank" rel="noopener noreferrer">
                                                                    <div className="nexter-download-template">
    ...
    return (
        <>
                ...
                <div className='tpae-widget-categories'>
                    <div className='tpae-cat-strip-cov-main'>
                        ...
                        <div className='theplus-right-search-and-btn-group'>
                            <div className='theplus-widget-search-box-cover'>
                                <img className='theplus-search-icon' src={plugin_url + 'assets/svg/search_icon.svg'} draggable={false} />
                                <input type='text' placeholder='Search' className='theplus-widget-search-box' onChange={(e) => { setsearch_filter(e.target.value) }} />
                            </div>
                        </div>
                    </div>
            ...
            {showPopup &&
                <ProNotice
                    heading='Upgrade to TPAE PRO to Unlock All Widgets & All Pre-Made Templates'
                    parent_class='tpae-pro-required'
                    inner_class='theplus_whitlabel_popup tpae-pro-required-inner'
                    close_svg=<svg width="25" height="25" viewBox="0 0 25 25" fill="none" xmlns="http://www.w3.org/2000/svg" onClick={() => { setShowPopup(false); }}>
                        <path d="M18.75 6.25L6.25 18.75" stroke="white" strokeWidth="0.75" strokeLinecap="round" strokeLinejoin="round" />
                        <path d="M6.25 6.25L18.75 18.75" stroke="white" strokeWidth="0.75" strokeLinecap="round" strokeLinejoin="round" />
                    </svg>
                />
            }
        </>
