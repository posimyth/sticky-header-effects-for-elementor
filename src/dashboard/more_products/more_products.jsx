...
const MoreProducts = (props) => {
    ...
    const PluginStatusBtn = (name, slug) => {        
        ...
        if (Btnloader[name] === 'unavailable') {
            return (
                <button className='tpae-install-btn' onClick={() => { Get_install_list(name, slug) }}>{__('Install & Activate', 'tpebl')}</button>
            )
        } else if (Btnloader[name] === 'inactive') {
            return (
                <button className='tpae-install-btn' onClick={() => { Get_install_list(name, slug) }}>{__('Activate Now', 'tpebl')}</button>
            )
        } else if (Btnloader[name] === 'loading') {
            return (
                <button className='tpae-install-btn'>
                    <span className='tpae-loading-circle'></span>
                </button>
            )
        } else {
            return (
                <button className='tpae-install-btn theplus-success'>
                    {__('Activated', 'tpebl')}
    ...
    return (
        <div className={`theplus_products_cover_main she-main-container ${props?.tpae_dashboard_data?.success ? '' : 'she-skeleton'}`}>
            ...
            {Object.entries(productsData).map(([key, product]) => {
                ...
                return (
                    <div key={key} className='theplus_products_box_cover_main'>
                        <div className='theplus_product_box'>
                            <div className='theplus_product_logo_title_cover'>
